# Kintone MCP サーバー環境構築スクリプト
# 実行方法: PowerShell 管理者権限で実行
...
# 変数設定
$mcpbPath = "Z:\kintone\kintone-mcp-server.mcpb"
$extractPath = "Z:\kintone\kintone-mcp-server-extracted"
$envFilePath = "$extractPath\.env"
$configPath = "$env:APPDATA\Claude\claude_desktop_config.json"

# ステップ1: .mcpb ファイルの存在確認
Write-Host "`n[ステップ1] .mcpb ファイルを確認中..." -ForegroundColor Yellow
if (-Not (Test-Path $mcpbPath)) {
    Write-Host "❌ エラー: $mcpbPath が見つかりません" -ForegroundColor Red
    exit 1
}
Write-Host "✅ ファイルを確認しました: $mcpbPath" -ForegroundColor Green

# ステップ2: 解凍フォルダが存在する場合は削除
...
# ステップ3: .mcpb ファイルを解凍
Write-Host "`n[ステップ3] .mcpb ファイルを解凍中..." -ForegroundColor Yellow
try {
    Expand-Archive -Path $mcpbPath -DestinationPath $extractPath -Force
...
# ステップ5: 依存関係をインストール
Write-Host "`n[ステップ5] 依存関係をインストール中..." -ForegroundColor Yellow
cd $extractPath
try {
    npm install --silent
...
# パスをエスケープ（backslash を double backslash に）
